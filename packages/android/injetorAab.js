// Injeta o conteúdo do dicionário (HTML + mídias + ícones) no template AAB.
// Usa fflate para manipulação do ZIP e Canvas API para redimensionar ícones.
// O template.aab deve ser um ZIP sem conteúdo real em base/assets/.
...
export class InjetorAab {
    /**
     * @param {Uint8Array} templateBytes - bytes do template.aab
    ...
    async injetar(templateBytes, conteudo, meta) {
        ...
        for (const path of Object.keys(arquivos)) {
            if (path.startsWith('base/assets/')) delete arquivos[path];
        }

        // 3. Injetar HTML principal
        arquivos['base/assets/index.html'] = [conteudo.htmlBytes, { level: 0 }];

        // 4. Injetar mídias mantendo caminhos relativos
        for (const [caminho, bytes] of conteudo.midias) {
            arquivos[`base/assets/${caminho}`] = [bytes, { level: 0 }];
        }
        ...
        if (conteudo.iconeBytes && conteudo.iconeBytes.length > 0) {
            for (const [size, density] of Object.entries(DENSITY_MAP)) {
                try {
                    ...
                    const prefix = `base/res/mipmap-${density}`;
        ...
        const manifestKey = 'base/manifest/AndroidManifest.xml';
        if (arquivos[manifestKey]) {
            ...
            const patchado = patchManifestAab(manifestBytes, {
